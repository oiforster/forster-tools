#!/usr/bin/env node
/**
 * gerar_pdf_nfse_ipm.js — Forster
 * ================================
 * Gera PDF de NFS-e emitida via WebService IPM (Prefeitura de Igrejinha/RS).
 * Usa o mesmo layout visual do gerar_pdf_nfse.js (Portal Nacional),
 * adaptado para os campos retornados pelo IPM.
 *
 * O XML de retorno IPM contém apenas: número, série, data/hora, status,
 * link e código verificador. Os demais dados (prestador, tomador, serviço,
 * valor, competência) são lidos de um arquivo JSON sidecar gerado na emissão.
 *
 * Uso:
 *   node gerar_pdf_nfse_ipm.js /caminho/para/_dados.json
 *
 * Saída: imprime o caminho absoluto do PDF gerado no stdout.
 *
 * Dependências: puppeteer-core (local), Google Chrome instalado.
 * Última atualização: 2026-04-07 (ajustes legais DANFSe)
 */
...
function montarEnderecoTomador(tom) {
  const partes = [
    tom.logradouro && tom.numero ? `${tom.logradouro}, ${tom.numero}` : (tom.logradouro || ''),
    tom.complemento || '',
    tom.bairro      || '',
    tom.municipio && tom.uf ? `${tom.municipio}/${tom.uf}` : '',
    tom.cep ? `CEP ${formatarCEP(tom.cep)}` : '',
  ].filter(Boolean);
  return partes.join(' · ');
}

function montarEnderecoPrestador(pre) {
  const partes = [
    pre.logradouro && pre.numero ? `${pre.logradouro}, ${pre.numero}` : (pre.logradouro || ''),
    pre.bairro || '',
    pre.municipio && pre.uf ? `${pre.municipio}/${pre.uf}` : '',
    pre.cep ? `CEP ${formatarCEP(pre.cep)}` : '',
  ].filter(Boolean);
  return partes.join(' · ');
}

// ---------------------------------------------------------------------------
// Geração do HTML — mesmo layout do gerar_pdf_nfse.js
// ---------------------------------------------------------------------------

function gerarHtml(d) {
  const tipoDocTom = String(d.tomador.cnpj || '').replace(/\D/g, '').length > 0 ? 'CNPJ' : 'CPF';
  const docTom     = d.tomador.cnpj || d.tomador.cpf || '';
  const endTom     = montarEnderecoTomador(d.tomador);

  const endPrestador = montarEnderecoPrestador(d.prestador);
  const prestadorEndereco = [
    endPrestador,
    d.prestador.im ? `IM ${d.prestador.im}` : '',
  ].filter(Boolean).join(' · ');
...
<div class="page">
  ...
  <div class="header">
    ...
    <div class="nota-info">
      <div class="nota-num">Nº ${esc(d.numero_nfse)}</div>
      <div class="nota-sub">Emissão: ${esc(d.data_emissao)} às ${esc(d.hora_emissao)}<br>Série ${esc(d.serie_nfse)}</div>
      <span class="badge">NFS-e · ${esc(d.prefeitura)}</span>
    </div>
  </div>
  ...
  <div class="section">
    ...
    <div class="partes-grid">
      <div class="card">
        <div class="field-name">${esc(d.prestador.nome)}</div>
        <div class="field-cnpj">CNPJ ${esc(formatarCpfCnpj(d.prestador.cnpj))}</div>
        ${d.prestador.regime_tributario ? `<span class="field-regime">${esc(d.prestador.regime_tributario)}</span>` : ''}
        <div class="field-addr">${esc(prestadorEndereco)}</div>
      </div>
      <div class="card">
        <div class="field-name">${esc(d.tomador.nome)}</div>
        <div class="field-cnpj">${tipoDocTom} ${esc(formatarCpfCnpj(docTom))}</div>
        <div class="field-addr">${esc(endTom)}</div>
      </div>
    </div>
  </div>
  ...
  <div class="section">
    ...
    <div class="servico-card">
      <div class="servico-texto">${esc(d.servico.descricao)}</div>
      <div class="meta-row">
        <span class="meta-tag">Cód. serviço ${esc(d.servico.codigo_item)}</span>
        <span class="meta-tag">Alíquota ISS ${esc(d.servico.aliquota_iss)}%</span>
        ${d.servico.situacao_tributaria_descricao ? `<span class="meta-tag-ok">${esc(d.servico.situacao_tributaria_descricao)}</span>` : ''}
        ${d.servico.iss_retido === false || d.servico.iss_retido === 'false'
          ? `<span class="meta-tag">ISS não retido na fonte</span>`
          : d.servico.iss_retido
            ? `<span class="meta-tag">ISS retido na fonte</span>`
            : ''}
        ${d.servico.iss_valor != null ? `<span class="meta-tag">ISS ${esc(formatarValor(d.servico.iss_valor))}</span>` : ''}
      </div>
    </div>
  </div>
  ...
  <div class="section">
    ...
    <div class="local-row">
      <div class="local-item">
        ...
        <div class="local-val">${esc(d.prefeitura)}</div>
      </div>
      <div class="local-item">
        <div class="local-label">Prestação</div>
        <div class="local-val">${esc(d.tomador.municipio)}/${esc(d.tomador.uf)}</div>
      </div>
    </div>
  </div>
  ...
  <div class="valores-row">
    <div class="valor-bloco">
      ...
      <div class="valor-num">${esc(formatarValor(d.valor))}</div>
    </div>
    <div class="competencia-bloco">
      <div class="bloco-label">Competência</div>
      <div class="competencia-val">${esc(formatarCompetencia(d.competencia))}</div>
    </div>
  </div>

  <div class="footer">
    <div class="chave-label">Código verificador de autenticidade</div>
    <div class="chave-val">${esc(d.cod_verificador_autenticidade)}</div>
    <div class="consulta-link">
      Autenticidade em
      <a href="${esc(d.link_nfse)}">${esc(d.prefeitura.split('/')[0].toLowerCase())}.atende.net</a>
    </div>
  </div>
...
async function gerarPdf(dadosPath) {
  const dados   = JSON.parse(fs.readFileSync(dadosPath, 'utf-8'));
  const html    = gerarHtml(dados);
  // Garante que o PDF fica no mesmo diretório do _dados.json
  const outPath = path.join(
    path.dirname(dadosPath),
    path.basename(dadosPath).replace(/_dados\.json$/, '.pdf').replace(/\.json$/, '.pdf')
  );
...
if (!dadosPath) {
  process.stderr.write('Uso: node gerar_pdf_nfse_ipm.js /caminho/para/_dados.json\n');
  process.exit(1);
}

if (!fs.existsSync(dadosPath)) {
  process.stderr.write(`Arquivo não encontrado: ${dadosPath}\n`);
  process.exit(1);
}

gerarPdf(path.resolve(dadosPath))
  .then(outPath => {
